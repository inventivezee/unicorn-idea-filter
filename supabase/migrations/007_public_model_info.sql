-- Unicorn Idea Filter — expose which model scored each instrument and when.
-- Appends 4 public-safe columns to the public view (model name + timestamp
-- only; still no rationales, no founder data). Idempotent; includes the
-- cashcow column and the full view, so it supersedes 005 and 006 — run this
-- one even if you skipped those.
...
select
  ...
  -- Which model produced each instrument's analysis, and when (public-safe).
