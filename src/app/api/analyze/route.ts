...
export async function POST(request: Request) {
    ...
    if (mode === "full" && !subscribed && !byokForProvider) {
      ...
      if (caller.user) {
        ...
        if (!allowed) {
          return Response.json(
            {
              error: `You've used all ${FREE_ANALYSES_PER_DAY} free analyses today — upgrade for unlimited analyses and premium models.`,
              upgrade: true,
            },
            { status: 402 },
          );
        ...
        if (!allowed) {
          return Response.json(
            {
              error: `Anonymous visitors get ${ANON_ANALYSES_PER_DAY} analyses per day — sign in for ${FREE_ANALYSES_PER_DAY} free per day, or subscribe for unlimited.`,
              upgrade: true,
            },
            { status: 402 },
          );
  ...
  try {
    ...
    if (filter === "cashcow") {
      ...
      const raw = parseLastJSON<RawAnalysis>(result.texts);
      const response = normalizeCashCowAnalysis(raw, result.webSearches, provider, model);
      if (persistTo) {
