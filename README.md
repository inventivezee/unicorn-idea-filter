...
- **AI analysis** — one click (or the type-to-add flow) fills the metadata and the whole scorecard from your idea + founder background (Anthropic Claude — Sonnet 5 at medium effort by default, Fable 5 at xhigh for subscribers — or OpenAI models led by GPT-5.5 (medium effort free, xhigh for subscribers), selectable in Settings), optionally grounded with live web search. Founder-personal gates (10-year commitment, unfair advantages) are flagged for manual confirmation, never silently auto-answered.
...
- **Accounts** — sign in with a magic link (email) or Google. Anonymous use works too, with a daily analysis quota; sign in to raise it, subscribe ($19/month) for unlimited fair-use plus private ideas, Claude Fable 5, and xhigh reasoning on GPT-5.5.
...
- **CVs are AI-summarised and retained.** Upload a CV (PDF/DOCX/TXT) and the AI condenses it into a founder background you can edit. In cloud mode the original file and its extracted text are saved to admin-only storage and kept permanently — clearing your background in the UI does not remove them.
...
- **Private ideas are a subscriber feature.** Subscribers ($19/month) can mark any idea private, which removes it from the public feed. The subscription also unlocks Claude Fable 5 and xhigh reasoning on GPT-5.5 (free tier runs Sonnet 5 and GPT-5.5 at medium effort).
